---
title: "I Ran Ten AI Models on My Real Work. Most of Them Were a Waste of Time."
description: "Five real tasks, identical prompts, full receipts — and the models I'd actually pay for."
socialTitle: '10 AI models, 5 real tasks, full receipts'
...
There are a lot of benchmarks for AI models. They all ask the same kind of questions: how does it do tool calls, how does it do long context, how does it do *this*, how does it do *that*.

I wanted to know something different: **how do these things actually feel?**
...
The honest headline, up front: **most of them are a waste of time.** Side by side is exactly how you find that out — and it's also how you find out which ones you can't afford to lose.

## The battery

Five tests, in the shape of work I really do (run on 2026-08-17, out of the box):

1. **The weekly workflow** — browse Reddit for a contentious engineering-leadership thread, write a "both sides + my read" blurb, generate a diagram.
...
| **Build**  | ✓ **$60.49** 6/6    | ✗ $20.29           | ✗ $90.67      | ✓ **$60.49** 6/6 | ✗ $69.06      | ✗ $56.44           | ✗ $45.59    | ✗ $112.26   | ✓ **$60.49** 6/6 | ✓ **$60.49** 6/6 |
| **Jobs**   | ✓ 23 scored, 3 pass | ✗ 0 viable         | —             | ✗ 0 viable       | —             | ✗ 0 viable         | ✗ 0 viable  | ✗ 0 viable  | ✗ 0 viable       | ✗ 0 viable       |
| **Vision** | — not run           | ✓ via glm-5v-turbo | — not run     | ✓ saw it         | ✓ iterated 4× | ✓ deepest critique | ✓ saw flaws | ✓ saw flaws | ✓ saw flaws      | ✓ saw flaws      |

*On the build test, ✓ means the app landed every vendor inside the converged band (the PDF scoreboard uses a looser bar — ✓ = 4+ of 6 vendors in band; I'm using the stricter one). Full numbers below.*

## Test 1 — The weekly workflow: research vs. generic

The task was simple: find a contentious, highly upvoted engineering-leadership thread on Reddit, cover both sides, add "my read," and generate a diagram. The Philip DeFranco move, basically.

The split was immediate: **research vs. generic.**

deepseek found a live r/EngineeringManagers thread — "Hands-off engineering leaders are dooming their teams" — and argued with it specifically, in 3 minutes 13 seconds. mimo found the "personality tax" thread on r/ExperiencedDevs (744 upvotes, 406 comments) — a staff engineer told she "lacks leadership qualities" for using hedging language — and covered both sides of it. terra and sol both found real threads and cited their sources — sol with upvote counts and a date, plus a diagram. kimi and grok found real threads too.
...
Four of the ten apps landed every vendor inside the converged band: deepseek, mimo, sol, terra — all at $60.49. grok came close ($56.44, missing one vendor because its session filter read a fraction of the usage). kimi overshot ($69.06, reading deepseek and xiaomi above the band). And then the failures, each different: glm-4.5 never priced xai at all ($20.29). gpt-5.5 under-read the metered vendors ($45.59) — its first run, before its units bug was fixed, reported $293,534.88, delivered with the same confidence. luna still estimated ($112.26). glm-4.5-flash was the outlier — its fallback-rate path inflated deepseek and priced quota sessions as if they were cash ($90.67, one vendor in band out of six).

One line, because it's the whole point of the ground-truth check: **every app reported its number with the same confidence** — the one that was off by hundreds of thousands sounded as sure as the ones that nailed it.
...
The meta-point first: the deliverable isn't the shortlist, it's the **skill** — the repeatable pipeline. That's the shape of agentic work: not doing a task, but turning it into something that runs forever. The pattern that fell out, with names: **author the skill once with the model that's best at planning — grok or sol — and run it forever with the cheapest model that's good enough — deepseek.** Authoring is the one-time cost. Running is the recurring cost.

The results were a crapshoot — and that's the finding. deepseek scored 23 roles; three passed the filters (Suno in Los Angeles, Anduril, Descript — each verified live on the employer's ATS). Every other model that produced a shortlist scored between five and eight roles — and **zero of them were viable.** Same location and recency criteria applied uniformly to all: LA or remote-US, not stale beyond 45 days.
...
grok wrote the deepest critique: *"The 39-point gap is computed correctly and drawn at the right x-span. It is not visible as the story."* And then fixed all three of its own findings. mimo was fast and precise — it caught that its own y-axis mislabeled the measured bar as "perceived." kimi iterated four times, fixing an annotation overlap and a cramped negative label. The OpenAI line genuinely saw its own flaws: gpt-5.5 caught a label collision, terra caught that its 39-point claim was asserted but never *shown* on the chart, sol caught the story inversion — the prediction bar getting more visual weight than the belief-vs-measured comparison — and luna caught a clipped label and a wrong bracket span.

Then there was the new entrant. deepseek and glm-4.5 are text-only on zai, so GLM ran its vision model instead — glm-5v-turbo — and it was strikingly pixel-accurate: *"the 39-point gap existed only as text, not as geometry."* Its v2 replaced the symbolic arrow with a vertical 39-unit dimension bracket, drawn to scale and verified pixel by pixel.

The honest-encoding check mattered: the measured bar is negative, so it must extend left of zero, and the gap must actually be visible. All seven original models got the negative bar right. **The difference was process, not sight.** An independent reviewer ranked the v2 figures blind — best figure: grok. Most honest: mimo.
...
**Most of these models are a waste of time — for me, at least.** Side by side is exactly how you find that out, and it's also how you know which ones to keep. It's nice to have a second model in your pocket for the days your rates change or your main provider throttles you.
...
And the meta-lesson: **author with your best model, run with your cheapest.** The battery exists to tell you which model deserves which role in that equation — and the ground-truth check is how you find out which model is which.
...
* **52 sessions stayed unpriced in every app** — several models have no published rate cards, so those sessions are honestly marked, not guessed.

* **One job market, one chart type, one cost window.** Your mileage will vary, deliberately.

* **I didn't tune anything.** These are out-of-the-box results. Aggressive per-model tuning would be a different test — and a less fair one.

* **The portal bills aren't the ground truth** — free credits and sliding windows make the wallet a noisy witness. The calculated answer is the reference.

## The receipts

The full battery is published alongside this post: **[the 101-page PDF (2.1 MB)](/battery-post-raw-data.pdf)** — every prompt, every score, every bill. The exact prompts are all in the PDF — each section here summarizes what was run — so you can re-run the whole battery on your own stack.
...
The same prompt produces wildly different behavior — research or generic, right or a hundredfold off, with equal confidence. The receipts let you draw your own conclusions.
